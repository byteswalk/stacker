...
export const ANALYSIS_TABS = ["overview", "directories", "large-files"] as const;
...
export function matchedFreeBytes(request: ScanRequest, volumes: readonly VolumeInfo[]): number | null {
  ...
  return matched.length === selected.size
    ? matched.reduce((sum, volume) => sum + volume.freeBytes, 0)
    : null;
}
...
export function AnalysisTabs({ taskId, request }: { taskId: string; request: ScanRequest }) {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled(requests).then(([summaryResult, volumeResult, settingsResult]) => {
      ...
      if (volumeResult.status === "fulfilled") {
        setFreeBytes(matchedFreeBytes(request, volumeResult.value));
      }
      if (settingsResult.status === "fulfilled") {
        setLargeFileThreshold(Math.max(1, settingsResult.value.large_file_threshold_bytes));
      }
      setLoading(false);
    });
  ...
  if (loading) {
    return (
      <div className="space-analysis-state" aria-live="polite">
        <i className="ti ti-loader spin" />
        <span>{tr("正在读取分析结果…")}</span>
      </div>
    );
  }
  ...
  if (error || !summary) {
    return (
      <div className="space-analysis-state error" role="alert">
        <i className="ti ti-alert-triangle" />
        <span>{error ?? tr("本次空间分析结果不可用，请重新扫描。")}</span>
      </div>
    );
  }
  ...
  return (
    <section className="space-analysis-results">
      ...
      <div className="space-analysis-tab-panel" role="tabpanel">
        ...
        {activeTab === "directories" && (
          <DirectoryRanking taskId={taskId} roots={summary.rootNodes} />
        )}
        {activeTab === "large-files" && (
          <LargeFiles taskId={taskId} thresholdBytes={largeFileThreshold} />
        )}
      </div>
    </section>
  );
