...
Stacker 会检查常见开发生态的状态，包括 Python、Node、Go、Maven、Gradle、Rust、终端代理和开发缓存。体检结果会直接给出可处理项，例如包源仍在官方源、开发缓存偏高、Windows 临时目录过大等，用户可以从概览页跳转到对应页面处理。
...
![Settings and source catalog](assets/screenshots/settings.png)
...
![Backup history](assets/screenshots/history.png)
