...
fn read_snapshot(path: &Path) -> Result<StoredSnapshot, String> {
    ...
    if snapshot.schema_version != SNAPSHOT_SCHEMA_VERSION
        || snapshot.rule_version != SNAPSHOT_RULE_VERSION
    {
...
fn write_atomic(path: &Path, value: &StoredSnapshot) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or_else(|| "Snapshot directory is unavailable.".to_string())?;
...
pub fn save_completed(
    ...
    let entries = rows
        .into_iter()
        .filter_map(|(path, allocated_bytes)| {
            relative_path(&path, &summary.targets).map(|relative_path| SnapshotEntry {
                relative_path,
                allocated_bytes,
            })
        })
    ...
    prune_group(
        ...
        settings.snapshot_max_per_target,
    )?;
    Ok(Some(metadata))
}
...
fn prune_group(fingerprint: &str, retention_days: u16, max_count: u16) -> Result<(), String> {
    ...
    let mut snapshots = std::fs::read_dir(&directory)
        ...
        .filter_map(|entry| {
            read_snapshot(&entry.path())
                .ok()
                .map(|snapshot| (entry.path(), snapshot.metadata.created_at))
        })
        .collect::<Vec<_>>();
    snapshots.sort_by(|left, right| right.1.cmp(&left.1));
    for (index, (path, created)) in snapshots.into_iter().enumerate() {
        let expired = DateTime::parse_from_rfc3339(&created)
            .map(|value| value.with_timezone(&Utc) < cutoff)
            .unwrap_or(true);
        if expired || index >= usize::from(max_count) {
...
pub fn list() -> Result<Vec<SnapshotMetadata>, String> {
        ...
        .filter_map(Result::ok)
    {
        ...
        for file in std::fs::read_dir(group.path())
            ...
            .filter_map(Result::ok)
        {
            if let Ok(snapshot) = read_snapshot(&file.path()) {
                items.push(snapshot.metadata);
            }
        }
    }
...
pub fn compare(
    ...
    let before = base
        .entries
        .iter()
        .map(|entry| (entry.relative_path.as_str(), entry.allocated_bytes))
        .collect::<HashMap<_, _>>();
    let after = current
        .entries
        .iter()
    ...
    let mut rows = paths
        ...
        .filter_map(|path| {
            ...
            (before_bytes != after_bytes).then(|| SnapshotChangeRow {
                ...
                before_bytes,
                after_bytes,
                delta_bytes: i128::from(after_bytes)
                    .saturating_sub(i128::from(before_bytes))
    ...
    Ok(SnapshotComparison {
        delta_bytes: i128::from(current.metadata.allocated_bytes)
            .saturating_sub(i128::from(base.metadata.allocated_bytes))
...
pub fn delete(id: &str) -> Result<(), String> {
    std::fs::remove_file(snapshot_path(id)?).map_err(|error| error.to_string())
}
...
mod tests {
    ...
    fn fingerprint_is_order_independent() {
        assert_eq!(
            target_fingerprint(&["D:\\b".into(), "C:\\a".into()]),
            target_fingerprint(&["c:/a/".into(), "d:/b".into()])
        );
    }
    #[test]
    fn relative_entries_do_not_expose_target_prefix() {
        assert_eq!(
            relative_path(
                r"C:\Users\demo\project\target",
                &[r"C:\Users\demo\project".into()]
            ),
            Some("target".into())
        );
    }
